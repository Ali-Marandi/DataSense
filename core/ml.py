...
def dataset_fingerprint(df: pd.DataFrame, target: str, features: list[str]) -> str:
    """Create a stable schema/shape fingerprint without serialising cell values."""
    payload = {
        "rows": int(len(df)),
...
def detect_leakage_risks(df: pd.DataFrame, target: str, features: list[str], time_column: str | None = None) -> list[str]:
    ...
    if time_column and time_column in features:
        warnings.append(f"Time column '{time_column}' is included as a predictor; verify that it is causally available at prediction time.")
    return warnings
...
def train_regression(
    ...
    X_tr, X_te, y_tr, y_te, split_strategy = _split_data(
        X, y, test_size=test_size, random_state=42, time_values=time_values
    )
    ...
    fingerprint = dataset_fingerprint(df, target, features)
    return ModelResult(
        ...
        metrics={
            ...
            "Baseline RMSE": round(float(np.sqrt(mean_squared_error(y_te, baseline_prediction))), 5),
            "CV R2 (mean)": round(float(cv.mean()), 5),
            "CV R2 (std)": round(float(cv.std()), 5),
            "Train rows": len(X_tr), "Test rows": len(X_te),
        },
        table=table,
        note=("Chronological split used; test data is later than training data. " if split_strategy == "chronological" else "Random split uses a fixed seed. ")
        + ("Leakage review: " + " | ".join(warnings) if warnings else "No automatic leakage warning was triggered."),
...
def train_classification(
    ...
    metrics: dict[str, float | int | str] = {
        ...
        "Precision (weighted)": round(float(precision_score(y_te, pred, average="weighted", zero_division=0)), 5),
        "Recall (weighted)": round(float(recall_score(y_te, pred, average="weighted", zero_division=0)), 5),
        "F1 (weighted)": round(float(f1_score(y_te, pred, average="weighted", zero_division=0)), 5),
        "Classes": int(y.nunique()), "Train rows": len(X_tr), "Test rows": len(X_te),
    }
    ...
    return ModelResult(
        ...
        metrics=metrics,
        table=table,
        note=("Chronological split used. " if split_strategy == "chronological" else "Random split uses a fixed seed. ")
        + ("Leakage review: " + " | ".join(warnings) if warnings else "No automatic leakage warning was triggered."),
        predictions=pd.DataFrame({"Actual": y_te.to_numpy(), "Predicted": pred}),
        metadata={
            "dataset_fingerprint": dataset_fingerprint(df, target, features),
            "target": target, "features": list(features),
            "split_strategy": split_strategy, "test_size": test_size,
            "random_state": 42, "leakage_warnings": warnings,
            "model_parameters": model.get_params(),
        },
    )
...
def run_clustering(df: pd.DataFrame, features: list[str], k: int = 3) -> ModelResult:
    ...
    return ModelResult(
        ...
        metrics={"Silhouette score": round(float(sil), 5) if sil == sil else "n/a", "Inertia": round(float(model.inertia_), 4), "Rows clustered": len(X)},
        table=centers,
        note="Cluster centroids in standardised feature space; k-means uses a fixed seed.",
        predictions=pd.concat([X.reset_index(drop=True), pd.Series(labels, name="Cluster")], axis=1),
        metadata={"features": list(features), "k": k, "random_state": 42},
    )
...
def run_pca(df: pd.DataFrame, features: list[str], components: int = 2) -> ModelResult:
    X = df[features].apply(pd.to_numeric, errors="coerce").replace([np.inf, -np.inf], np.nan).dropna()
    if X.empty:
        raise ValueError("No complete numeric rows are available for PCA.")
    if components < 1:
    ...
    return ModelResult(
        ...
        metrics={"Total variance explained %": round(float(pca.explained_variance_ratio_.sum() * 100), 3), "Features": len(features), "Rows": len(X)},
        table=table,
        note="Explained variance is computed after standardising the selected numeric features.",
        predictions=pd.DataFrame(scores, columns=[f"PC{i + 1}" for i in range(components)]).round(5),
        metadata={"features": list(features), "components": components, "loadings": loadings.to_dict(orient="records")},
    )
