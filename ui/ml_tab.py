"""Machine learning workspace with background training."""
...
class MLTab(QWidget):
    ...
    def _build(self) -> None:
        ...
        grid.addWidget(QLabel("Clusters / components"), 1, 2)
        self.k_spin = QSpinBox()
        self.k_spin.setRange(2, 20)
        self.k_spin.setValue(3)
        grid.addWidget(self.k_spin, 1, 3)
        self.train_btn = QPushButton("Train model")
        self.train_btn.setProperty("accent", True)
        self.train_btn.clicked.connect(self.train)
        grid.addWidget(self.train_btn, 1, 5)
    ...
    def train(self) -> None:
        ...
        if task == "Regression":
            job = lambda: ml.train_regression(df, target, features, algo, size)
        elif task == "Classification":
            job = lambda: ml.train_classification(df, target, features, algo, size)
    ...
    def _append_predictions(self) -> None:
        ...
        if len(predictions) != len(self.manager.df):
            QMessageBox.information(
                self,
                "Row mismatch",
                "Predictions cover only complete rows, so they cannot be aligned with the "
                "full dataset. Remove missing values first, then retrain.",
            )
            return
        merged = pd.concat(
            [self.manager.df.reset_index(drop=True), predictions.reset_index(drop=True)], axis=1
        )
