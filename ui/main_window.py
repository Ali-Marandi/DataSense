...
from core.version import APP_NAME, APP_PUBLISHER, APP_TAGLINE, APP_URL, APP_VERSION
...
IMPORT_FILTER = (
    "All supported (*.csv *.tsv *.txt *.xlsx *.xls *.json *.parquet *.db *.sqlite);;"
    "CSV (*.csv *.txt *.tsv);;Excel (*.xlsx *.xls);;JSON (*.json);;Parquet (*.parquet);;"
    "SQLite (*.db *.sqlite)"
)
...
class MainWindow(QMainWindow):
    ...
    def _build_actions(self) -> None:
        ...
        self.act_import = action("Import data…", self.import_data, "Ctrl+O",
                                 "Import CSV, Excel, JSON, Parquet or SQLite data")
        self.act_sample = action("Load sample dataset", self.load_sample, "Ctrl+Shift+S",
                                 "Generate a demo dataset to explore the app")
        self.act_export = action("Export dataset…", self.export_data, "Ctrl+E",
                                 "Export the active dataset")
        self.act_open_project = action("Open project…", self.open_project, "Ctrl+Shift+O")
        self.act_save_project = action("Save project…", self.save_project_as, "Ctrl+S")
        self.act_report = action("Export HTML report…", self.export_report, "Ctrl+R",
                                 "Build a shareable report from this session")
        self.act_undo = action("Undo", self.undo, "Ctrl+Z")
        self.act_redo = action("Redo", self.redo, "Ctrl+Y")
        self.act_theme = action("Toggle light / dark theme", self.toggle_theme, "Ctrl+T")
        self.act_about = action(f"About {APP_NAME}", self.show_about, "F1")
        self.act_quit = action("Exit", self.close, "Ctrl+Q")
